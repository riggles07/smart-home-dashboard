...
module.exports = function (RED) {
    ...
    class HubitatNode extends RED.Node {
        ...
        makerApiRequest(url, meta, callback) {
            ...
            try {
                // The transport may be synchronous (as in test harness) or async.
                // Capture the result from the callback and return it for callers that expect a return value.
                let capturedResult = undefined;
                const transportResult = this.sendRequest('GET', url, null, (err, response) => {
                    const status = response && response.statusCode;
                    if (err) {
                        const text = String((err && err.message) || err);
                        const timedOut = err.code === 'ETIMEDOUT' || err.code === 'ESOCKETTIMEDOUT' || /timeout/i.test(text);
                        capturedResult = done(this.fail(timedOut ? 'timeout' : 'connection_error',
                ...
                // If the transport returns a non‑undefined value (some sync transports
                // directly return a result object), prefer that. Otherwise, return the
